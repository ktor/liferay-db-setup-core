...
. Number the pages you care about with contiguous values starting at `0`.
. Leave the attribute out on pages whose position should not change - a page without `page-sequence` keeps its current position, so an ordering done manually in the portal is not overwritten on the next setup run.
